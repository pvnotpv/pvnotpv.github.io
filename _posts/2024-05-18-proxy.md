...
PS: The sole reason why I'm writing this post is because there's not much proper guides out there that's actually implementing these different types of proxy standards.
...
- The delegatecall function runs the code in the logic contract as it's own code meaning msg.sender and msg.value is going to be the same.
...
In the case of uups proxy the upgrade mechanism is included in the implementation contract but not in the proxy contract itself. Check out the comparison at the end to find out why.
...
contract MyProxy is ERC1967Proxy {
    ...
    function upgrade(address _implemenation, bytes memory data, Bank bank) public {
        address(bank).delegatecall(abi.encodeWithSignature("upgradeToAndCall(address,bytes)", _implemenation, data));
    }

}
```

Bank contract(Our implementation contract):
