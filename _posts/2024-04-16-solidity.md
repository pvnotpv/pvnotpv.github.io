...
When delegatecall is invoked the pwn() function is run it belongs to the Delegation contract so the owner variable is going to changed for the Delegation contract not for Delegate.
...
```
- This function returns the abi of pwn() function which can be passed to the calldata to call the pwn() function.
